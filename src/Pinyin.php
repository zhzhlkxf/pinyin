...
class Pinyin
{
    ...
    protected function romanize($string, $option = \PINYIN_DEFAULT)
    {
        ...
        if ($this->hasOption($option, \PINYIN_NUMBER)) { // 数字转拼音
            $string = $this->convertNumberName($string, $dictLoader);
        }
    ...
    /**
     * Convert Chinese Numbername to pinyin.
     * @param $string
     * @param $dictLoader
     * @return mixed
     */
    protected function convertNumberName($string, $dictLoader)
    {
        $dictLoader->mapNumberName(function ($dictionary) use (&$string) {
            foreach ($dictionary as $number => $pinyin) {
                $string = strtr($string, $dictionary);
            }
        });
    ...
    {
        $string = preg_replace_callback('~[0-9_-]+~i', function ($matches) { //~[a-z0-9_-]+~i
            return "\t".$matches[0];
        }, $string);
        $string = preg_replace_callback('~[a-z_-]+~i', function ($matches) { // 将字母单独处理
            return "\t".$matches[0];
        }, $string);
